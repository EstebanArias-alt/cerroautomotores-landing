...
<section id="prestamos" class="scroll-mt-24 py-16 bg-brand-dark text-white">
  <div class="mx-auto max-w-4xl px-4">
    ...
    <p class="text-brand-gray mt-2">Calculá una cuota aproximada según monto, cuotas y periodicidad. Tasa por periodo: <b>Quincenal 15%</b> · <b>Diaria 2%</b>.</p>
    ...
    <div *ngIf="valorCuota" class="mt-5 rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
      ...
      <div class="mt-1 text-white/70 text-sm">
        Total a pagar: <b>{{ total | number:'1.0-0' }}</b>
        <span class="text-white/50"> (capital + interés simple en {{ cuotas }} {{ periodicidad }})</span>
      </div>
    </div>
...
export class BusinessLoanComponent {
  ...
  private readonly tasaQuincena = 0.15; // 15% por quincena
  private readonly tasaDiaria   = 0.02; // 2% por día
  ...
  calc() {
    ...
    // Interés simple acumulado por periodo
    this.total = m * (1 + r * n);
